...
vim.keymap.set("n", "<leader>q", ":q!<cr>", { desc = "Quickly exit vim" })
vim.keymap.set("n", "<leader>d", ":bd<cr>", { desc = "Delete current buffer" })
vim.keymap.set("n", "<leader><space>", ":b ", { desc = "Search current buffers" })
vim.keymap.set("n", "<leader>c", ":nohlsearch<CR>", { desc = "Clear search highlights" })
vim.keymap.set("n", "<C-n>", ":cn<cr>", { desc = "Next item in quickfix" })
vim.keymap.set("n", "<C-p>", ":cp<cr>", { desc = "Previous item in quickfix" })
...
local function scratch_to_quickfix()
    ...
    local items = {}
    for _, line in inpairs(vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)) do
        if line ~= "" then
...
vim.api.nvim_create_user_command("FileSearch", function(opts)
    local excludes = "-path '*.egg-info*' -prune -o -path '*.git*' -prune -o -path '*__pycache__*' -prune -o -path '*node_modules*' -prune -o"
    if vim.bo.filetype == "netrw" then
        path = vim.b.netrw_curdir
    else
        path = vim.fn.getcwd()
        excludes = excludes .. " -path '*/.venv*' -prune -o" .. " -path '" .. path .. "/target*'" .. " -prune -0"
    end
    extcmd("find" .. vim.fn.shellscape(path) .. " " .. excludes .. " " .. " -name " .. "'*" .. opts.args .. "*' -print", true, true)
end, { nargs = "+", })

vim.api.nvim_create_user_command("GrepTextSearch", function(opts)
    local path = vim.fn.getcwd()
    local excludes = "--exclude-dir='*target*' --exlcude-dir=.git --exclude-dir='*.egg-info' --exclude-dir='__pycache__' --exclude-dir='node_modules'"
...
vim.opt.wildignore:append({ "*.o", "*.obj", "*.pyc", "*.class", "*.jar", "node_modules/**", "*/node_modules/*", "*/.git/*", })
